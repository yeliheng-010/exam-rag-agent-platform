...
func BuildContainer(container *dig.Container) *dig.Container {
	...
	must(container.Provide(service.NewExamInterventionService))
	...
	must(container.Provide(service.NewExamAgentEvaluationService))
	must(container.Provide(service.NewExamEvaluationCenterService))
	...
	must(container.Provide(func(s interfaces.KnowledgeService) interfaces.ExamMaterialKnowledgeReader {
		return s
	}))
	must(container.Provide(func(s interfaces.ChunkService) interfaces.ExamMaterialChunkReader {
		return s
	}))
	must(container.Provide(service.NewExamMaterialService))
	...
	must(container.Provide(handler.NewExamInterventionHandler))
	...
	must(container.Provide(handler.NewExamAgentEvaluationHandler))
	must(container.Provide(handler.NewExamEvaluationCenterHandler))
	...
	must(container.Provide(handler.NewExamMaterialHandler))
