...
type fakeMemberService struct {
	...
	// that resolveTenantRole did (or didn't) attempt to auto-heal.
	addCalls []struct {
		UserID   string
		TenantID uint64
	...
	failHasAny  error
	failAdd     error
	// 阻止 auto-heal 把 hasAny 翻面：默认 AddMember 成功也会写入 members map。
}
...
func TestResolveTenantRole_ActiveMembershipWins(t *testing.T) {
	...
	if len(svc.addCalls) != 0 {
		t.Fatalf("must not auto-heal when membership exists, got %d AddMember calls", len(svc.addCalls))
	}
}

func TestResolveTenantRole_CrossTenantSuperuserGetsAdmin_NoAutoHeal(t *testing.T) {
	...
	if len(svc.addCalls) != 0 {
		t.Fatalf("cross-tenant superuser must not trigger auto-heal, got %+v", svc.addCalls)
	}
}

func TestResolveTenantRole_AutoHealRequiresHomeTenant(t *testing.T) {
	// 回归 H1：即便 target 是孤儿租户，只要不是用户自己的 home tenant，
	// 就不能 auto-heal 出 membership。
	...
	if len(svc.addCalls) != 0 {
		t.Fatalf("auto-heal must skip cross-tenant target, got %+v", svc.addCalls)
	}
}

func TestResolveTenantRole_AutoHealHomeTenant(t *testing.T) {
	// home tenant + 孤儿租户 + 非 switch → 自动补 Viewer membership。
	svc := newFakeMemberService()
	...
	if !ok || got != types.TenantRoleViewer {
		t.Fatalf("got (%v, %v), want (viewer, true)", got, ok)
	}
	if len(svc.addCalls) != 1 || svc.addCalls[0].Role != types.TenantRoleViewer {
		t.Fatalf("expected exactly one Viewer AddMember call, got %+v", svc.addCalls)
	}
}

func TestResolveTenantRole_AutoHealSkippedIfTenantHasMembers(t *testing.T) {
	svc := newFakeMemberService()
	...
	if len(svc.addCalls) != 0 {
		t.Fatalf("must not auto-heal into a tenant that already has members, got %+v", svc.addCalls)
	}
}
...
func TestResolveTenantRole_FailOpenAdminWhenRBACDisabled(t *testing.T) {
	svc := newFakeMemberService()
	user := &types.User{ID: "u1", TenantID: 7}
	// targetTenantID != home，所以不进 auto-heal 分支。
...
func TestResolveTenantRole_LookupErrorFailsOpenWhenRBACDisabled(t *testing.T) {
	...
	// 选与 home 不同的值，避免进入 home-tenant auto-heal 分支。
	svc := newFakeMemberService()
...
func TestResolveTenantRole_DemotedUserCannotReclaimViaOrphan(t *testing.T) {
	...
	// 当前策略是 "home tenant + 孤儿 => Viewer"，保留可登录性但不恢复管理权限。
	svc := newFakeMemberService()
	user := &types.User{ID: "demoted", TenantID: 5}
	got, ok := resolveTenantRole(context.Background(), svc, user, 5, false, cfgWithRBAC(true))
	if !ok || got != types.TenantRoleViewer {
		t.Fatalf("current policy heals orphan home tenant as viewer, got (%v, %v)", got, ok)
	}
}
